...
import { adaptCall, adaptCallList } from './adapters/callAdapter';

export default function Auditor22Page() {
  ...
  const syncWithDrive = async () => {
    ...
    try {
      ...
      const data = await apiClient.get<{ calls: any[] }>('/drive-history', {
      ...
      if (data.calls && Array.isArray(data.calls)) {
        setCalls(prev => {
          ...
          data.calls.forEach((driveCall: any) => {
            const adapted = adaptCall(driveCall);
            if (!existingIds.has(adapted.id)) {
              combined.push({
                ...adapted,
                isFromDrive: true,
              });
  ...
  // Cargar llamadas reales de forma dinámica al montar el componente y fusionar con el caché local del dispositivo
  useEffect(() => {
    setIsLoading(true);
    ...
    // 2. Cargar del servidor (JWT vía apiClient)
    apiClient
      .get<any[]>('/llamadas')
      .then(serverRaw => {
        const serverData = adaptCallList(serverRaw);
      ...
      .catch(err => {
        console.error("Usando caché local:", err);
  ...
  // Guardar en caché local cuando cambien las llamadas de forma reactiva (excepto en estado de carga)
  useEffect(() => {
    if (!isLoading) {
      try {
  ...
  const handleImportFromDrive = async (fileId: string, fileName: string) => {
    ...
    try {
      const completedCall = await apiClient.post('/drive-import', {
        fileId,
        fileName,
        accessToken: driveToken,
      ...
      handleUploadSuccess(adaptCall(completedCall));
    } catch (err) {
  ...
  const handleUploadSuccess = async (newCall: SalesCall) => {
    ...
    if (driveToken) {
      try {
        await apiClient.post('/drive-save', {
          callData: newCall,
          accessToken: driveToken,
        });
  ...
  const handleDeleteCall = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await apiClient.delete(`/llamadas/${id}`);
    } catch (err) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#0a0a0a] text-gray-200 font-sans selection:bg-indigo-950 selection:text-indigo-200 pb-12">
      {/* Main Area */}
                              ...
                              <button
                                ...
                                className={`p-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 ${
                                  isActive
                                    ...
                                    : 'hover:bg-zinc-800 text-gray-400 hover:text-rose-400'
                                }`}
