...
export default function AudioUpload({ onUploadSuccess }: AudioUploadProps) {
  ...
  const handleDriveImport = async () => {
    let accessToken = getDriveToken();
    
    if (!accessToken) {
      setUploadError(null);
      setIsDriveLoading(true);
      try {
        const { googleSignIn } = await import('../lib/googleDriveToken');
  ...
  const importFileFromDrive = async (fileId: string, fileName: string) => {
    ...
    try {
      ...
      onUploadSuccess(adaptCall(completedCall));
      setUploadProgress(null);
    } catch (err: any) {
      setUploadError(err?.response?.data?.error || err?.message || 'Error al importar de Drive.');
      setUploadProgress(null);
    }
  };
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      // El servidor devuelve { callId } y el análisis se completa vía polling.
      const callId: string = data.callId;
      if (!callId) {
        throw new Error('No se pudo obtener un ID de llamada del servidor.');
      }

      setUploadProgress(35);

      // Polling de transcripción + análisis
      let pollCount = 0;
      const maxPolls = 120;
      let completedCall: SalesCall | null = null;

      while (pollCount < maxPolls) {
        await new Promise(r => setTimeout(r, 5000));
        pollCount++;

        const statusData = await apiClient.get<{ status: string; call?: any; error?: string }>(
          `/transcript/${callId}`,
        );
        if (statusData.status === 'completed') {
          completedCall = adaptCall(statusData.call);
          break;
        } else if (statusData.status === 'error') {
          throw new Error(statusData.error || 'Error en transcripción');
        }

        setUploadProgress(Math.min(50 + Math.floor(pollCount * 0.4), 92));
      }
      ...
      setTimeout(() => {
        onUploadSuccess(completedCall as SalesCall);
        setUploadProgress(null);
