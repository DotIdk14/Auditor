...
export default function App({ session, apiUrl, onLogout }: AuditoriasMainProps) {
  ...
  const syncWithDrive = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/drive-history?accessToken=${driveToken}`);
  ...
  const handleLoadDemo = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${apiUrl}/api/cargar-demo`, {
        method: 'POST'
      });
  ...
  const handleDeleteCall = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await fetch(`${apiUrl}/api/llamadas/${id}`, { method: 'DELETE' });
    } catch (err) {
